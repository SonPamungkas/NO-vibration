...
public class NOCV: BaseUnityPlugin
{
    ...
    private void Awake()
    {
        AppDomain.CurrentDomain.AssemblyResolve += (sender, args) =>
        {
            ...
            if (assemblyName.Name == "websocket-sharp")
            {
                var localPath = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(Info.Location), "websocket-sharp.dll");
            ...
            {
                var localPath = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(Info.Location), "Newtonsoft.Json.dll");
                if (System.IO.File.Exists(localPath))
                {
                    return System.Reflection.Assembly.LoadFrom(localPath);
                }
